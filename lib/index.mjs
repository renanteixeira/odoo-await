...
// Dynamic import for xmlrpc to avoid browser issues
let xmlrpcCache = null;

async function getXmlrpc() {
  if (!xmlrpcCache) {
    xmlrpcCache = await import('xmlrpc');
  }
  return xmlrpcCache;
}
...
class OdooAwait {
  ...
  constructor (opts) {
    ...
    if (typeof window !== 'undefined') {
      throw new Error(
        'OdooAwait is designed for Node.js backend environments only. ' +
        'It cannot run in browsers due to XML-RPC protocol requirements. ' +
        'For frontend applications, consider using Odoo\'s REST API or GraphQL alternatives, ' +
        'or implement server-side API routes that use this library.'
      );
    }
    ...
    if (!options.db || typeof options.db !== 'string' || options.db.trim() === '') {
  ...
  async connect(){
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error('Connection timeout: Unable to connect to Odoo server'));
      ...
      client.methodCall('authenticate', params, function(err, uid) {
        ...
        if(err){
          ...
          reject(new Error(`Connection error: ${sanitizedMessage}`));
          return;
        }
        if(!uid){
          reject(new Error("Authentication failed: Invalid credentials or database name"));
          return;
        }
        if(typeof uid !== 'number' || uid <= 0){
          reject(new Error("Authentication failed: Invalid user ID received"));
          return;
        }
        self.uid = uid;
        resolve(uid);
  ...
  async create(model, params = {}, externalId, moduleName){
    ...
    if (!params || typeof params !== 'object' || Array.isArray(params)) {
      throw new Error('Parameters object is required and must be a valid object');
    }

    if (externalId && (typeof externalId !== 'string' || externalId.trim() === '')) {
      throw new Error('External ID must be a non-empty string if provided');
    }

    if (moduleName && (typeof moduleName !== 'string' || moduleName.trim() === '')) {
      throw new Error('Module name must be a non-empty string if provided');
    }

    params = this.parseManyFields(params);
    const recordId =  await this.execute_kw(model.trim(), 'create', [[params]]);

    // create external ID if it was supplied
    if(externalId){
      await this.createExternalId(model.trim(), recordId, externalId.trim(), moduleName || '__api__')
    }
  ...
  async read(model, recordId, fields = []) {
    ...
    // Validate recordId is a number or array of numbers
    if (Array.isArray(recordId)) {
      if (recordId.length === 0) {
        throw new Error('Record ID array cannot be empty');
      }
      for (const id of recordId) {
        if (!Number.isInteger(id) || id <= 0) {
          throw new Error('All record IDs must be positive integers');
        }
      }
    } else {
    ...
    if (fields && !Array.isArray(fields)) {
      if (typeof fields === 'string') {
        fields = [fields]; // Convert single field string to array
      } else {
    ...
    return await this.execute_kw(model.trim(), 'read', params);
  ...
  async update(model, recordId, params = {}) {
    ...
    if (typeof recordId !== 'number' && !Array.isArray(recordId)) {
      throw new Error('Record ID must be a number or array of numbers');
    }

    if (Array.isArray(recordId) && recordId.some(id => typeof id !== 'number' || !Number.isInteger(id) || id <= 0)) {
      throw new Error('All record IDs must be positive integers');
    }

    if (!Number.isInteger(recordId) && typeof recordId === 'number' && recordId <= 0) {
      throw new Error('Record ID must be a positive integer');
    }
  ...
  async searchRead(model, domain, fields = [], opts = { offset: 0, limit: 0, order: '', context: {}}) {
    ...
    if (opts.offset !== undefined) {
      if (!Number.isInteger(opts.offset) || opts.offset < 0) {
        throw new Error('Offset must be a positive number or zero');
      }
    }
